...
@section('title', __('messages.compare_products') . ' - Technorics')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2 text-sm">
                ...
                <span class="text-gray-900 dark:text-white font-semibold">{{ __('messages.compare_products') }}</span>
            </div>
            @if($products->count() > 0)
...
<div class="bg-gray-50 dark:bg-gray-900 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="text-center py-16">
            ...
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ __('messages.no_products_compare') }}</h2>
        ...
        @else
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ __('messages.compare_products') }}</h1>
            <p class="text-gray-600 dark:text-gray-400">{{ __('messages.compare_up_to_4') }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-white sticky left-0 bg-gray-50 dark:bg-gray-700">{{ __('messages.specifications') }}</th>
                        ...
                        <th class="px-6 py-4 text-center min-w-[250px]">
                            <div class="flex flex-col items-center">
                                ...
                                <form action="{{ route('compare.remove', $product) }}" method="POST" class="mt-2">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-700 text-sm">{{ __('messages.remove') }}</button>
                                </form>
                            </div>
                        </th>
                        @endforeach
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800">{{ __('messages.price') }}</td>
                        @foreach($products as $product)
                    ...
                    <!-- Rating -->
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800">{{ __('messages.rating') }}</td>
                        @foreach($products as $product)
                        <td class="px-6 py-4 text-center">
                            ...
                            <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $product->rating }}/5 ({{ $product->reviews_count }} {{ __('messages.reviews') }})</div>
                        </td>
                        @endforeach
                    </tr>
                    ...
                    <!-- Brand -->
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800">{{ __('messages.brand') }}</td>
                        @foreach($products as $product)
                    ...
                    <!-- Stock -->
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800">{{ __('messages.availability') }}</td>
                        @foreach($products as $product)
                        <td class="px-6 py-4 text-center">
                            @if($product->stock > 0)
                    ...
                    <tr class="bg-gray-50 dark:bg-gray-700">
                        <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white sticky left-0 bg-gray-50 dark:bg-gray-700">{{ __('messages.actions') }}</td>
                        @foreach($products as $product)
                        <td class="px-6 py-4 text-center">
